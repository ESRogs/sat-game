...
/-- Check if a literal would become true when we assign a specific value to a specific variable -/
...
/-- If a literal becomes true under var := value, then it's satisfied by any assignment extending this -/
theorem becomesTrueUnder_implies_satisfiedBy_extension {Var : Type} [DecidableEq Var]
  ...
  rw [Bool.and_eq_true] at h_becomes_true
  obtain ⟨h_contains, h_eval⟩ := h_becomes_true
