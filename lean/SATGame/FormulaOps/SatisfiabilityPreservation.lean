...
theorem filtered_clause_satisfaction_extends {Var : Type} [DecidableEq Var]
    ...
    processClause orig_clause var value = some filtered_clause →
  ...
  have h_lit_in_orig : List.Mem lit orig_clause := by
    -- We know h_process : processClause orig_clause var value = some filtered_clause
    -- and h_lit_in_filtered : lit ∈ filtered_clause
    unfold processClause at h_process
    split at h_process
  ...
  have h_extended_satisfies_original : formula.satisfiedByAssignment extended_α = true := by
    ...
    · -- Case 2: Clause was filtered and appears in result
      ...
      have h_filtered_exists : ∃ filtered_clause,
          processClause clause var value = some filtered_clause ∧
...
theorem setVariable_preserves_satisfiability_under_witness {Var : Type} [DecidableEq Var]
  ...
  -- Analyze how processClause processed this clause
  unfold processClause at h_process_result

  -- h_process_result says processClause returned some
  -- This means the clause was NOT satisfied by var alone
  split at h_process_result
  · -- Case: clause was satisfied, contradiction since we got some not none
    simp at h_process_result
  · -- Case: clause was not satisfied, so it was filtered
    simp at h_process_result
    rw [← h_process_result]
