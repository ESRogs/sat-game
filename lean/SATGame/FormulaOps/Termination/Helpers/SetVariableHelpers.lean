...
-- Basic helper: When a clause is satisfied, processClause returns none
theorem processClause_satisfied_returns_none {Var : Type} [DecidableEq Var]
    ...
    processClause clause var value = none := by
  unfold processClause
  rw [if_pos h_satisfied]

-- Basic helper: When a clause is not satisfied, processClause returns a clause without the variable
theorem processClause_unsatisfied_removes_variable {Var : Type} [DecidableEq Var]
    ...
    ∃ result_clause, processClause clause var value = some result_clause ∧
...
theorem filter_removes_element_strict {α : Type} (l : List α) (p : α → Bool) (x : α)
  ...
  have h_pos : 0 < (l.filter (fun a => ¬p a)).length := by
    by_contra h_not_pos
    -- h_not_pos : ¬(0 < (l.filter (fun a => ¬p a)).length)
    -- This means (l.filter (fun a => ¬p a)).length ≤ 0, which means it equals 0
...
private theorem processed_clause_no_variable_when_original_contained {Var : Type} [DecidableEq Var]
    ...
    (h_process_result : processClause orig_clause var value = some clause) :
...
private theorem process_clause_when_no_variable {Var : Type} [DecidableEq Var]
    ...
    (h_process_result : processClause orig_clause var value = some clause) :
    Clause.containsVariable clause var = false := by
  unfold processClause at h_process_result
...
theorem setVariable_removes_variable {Var : Type} [DecidableEq Var]
  ...
  have h_exists : ∃ orig_clause, List.Mem orig_clause formula ∧ processClause orig_clause var value = some clause := by
  ...
  · -- Case 2: Original didn't contain the variable (h_contains gives us false directly)
