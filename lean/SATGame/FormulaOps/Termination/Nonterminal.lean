...
theorem nonterminal_contains_variable {Var : Type} [DecidableEq Var] (formula : Formula Var)
  ...
  obtain ⟨clause, h_clause_in⟩ : ∃ (clause : Clause Var), List.Mem clause formula :=
  ...
  obtain ⟨literal, h_literal_in⟩ : ∃ (literal : Literal Var), List.Mem literal clause :=
    ⟨clause.get ⟨0, h_clause_length⟩, List.get_mem clause ⟨0, h_clause_length⟩⟩

  -- Step 5: Extract variable from literal
  obtain ⟨var, h_var_in_literal⟩ : ∃ var, literal.containsVariable var = true :=
