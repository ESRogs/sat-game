...
theorem assignment_strategy_reaches_empty {Var : Type} [DecidableEq Var] [Inhabited Var]
  ...
  -- Use well-founded induction on the literal count
  have assignment_reaches_empty :
    ∀ (n : Nat) (f : Formula Var),
      f.countLiterals = n →
      f.satisfiedByAssignment assignment = true →
      game_reaches_terminal (assignment_to_strategy assignment) f [] := by
    intro n
    induction n using Nat.strong_induction_on with
    | h n ih =>
      intro f h_count h_f_satisfies
      ...
      · -- Nonterminal case: apply strategy and use induction
        ...
        have h_count_lt_n : h.countLiterals < n := by
          rw [← h_count]
          exact h_progress
        have h_reaches_empty := ih h.countLiterals h_count_lt_n h rfl h_h_satisfies

        -- Construct the game sequence: f → g → h → []
        exact game_reaches_terminal.step h_not_terminal rfl [] h_neg_valid h_h_eq h_reaches_empty

  exact assignment_reaches_empty initial.countLiterals initial rfl h_satisfies
